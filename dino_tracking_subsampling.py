...
# Output directory
EXPERIMENT_SAVE_DIR = "/home/esli/GenParticles_NeurIPS/TAPVID_subsampling_exp2"
...
def blob_tracking_gibbs_dino(key, hdgmm_state):
    ...
    #     key, gibbs_key = jax.random.split(key)
    #     hdgmm_state = gibbs_blob_assignments_dino(
    #         gibbs_key, hdgmm_state, feature_only=True, disable_outlier_prob=False
    #     )
    #     key, gibbs_key = jax.random.split(key)
    #     hdgmm_state = gibbs_blob_weights(gibbs_key, hdgmm_state)
    #     return key, hdgmm_state
    ...
    try:
        ...
        if subsampled_indices is None and subsampling_percentage < 100.0:
            tracked_points, tracked_motion_vectors, subsampled_indices = sample_datapoints_percentage(
                tracked_points, tracked_motion_vectors, subsampling_percentage, seed=RANDOM_SEED, same_indices_all_timesteps=True
            )
            # Subsample DINO features with same indices
            tracked_features = tracked_features[:, subsampled_indices, :]
        ...
        error_results = compute_error_rates(
            tracking_data, segmentation_masks, img_dims,
                ...
                subsampled_indices=subsampled_indices, is_mask_subsampled=is_mask_subsampled
            )
...
if __name__ == "__main__":
    ...
    # subsampling_percentages = [50.0, 6.25, 3.125]
        ...
        for percentage in subsampling_percentages:
            ...
            if result is not None:
                ...
                create_3wide_video(
                    ...
                    NUM_BLOBS, video_path, subsampled_indices=result.get('subsampled_indices'), is_subsampled=is_subsampled
                )
