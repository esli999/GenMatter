# GenMatter Tracking with DINO Features — DAVIS ablation-3 (ablation-2 + frozen hyperblob Gibbs during tracking).
# See run_davis_ablation_2.py for the large-Psi_H recipe; run_davis_ablation.py for ablation-1.
...
EXPERIMENT_SAVE_DIR = str(config.DAVIS_ABLATION_3_OUTPUT_DIR_SAM)

# Model hyperparameters
NUM_BLOBS = 500
# Ablation-3: same init/Ψ_H as ablation-2 but no hyperblob Gibbs during tracking (hyperblobs frozen after init+init Gibbs).
...
def blob_tracking_gibbs_dino(key, genmatter_state):
    """Single-frame Gibbs updates (ablation-3: hierarchical blob mean/vel; hyperblob Gibbs frozen during tracking)."""
    ...
    # Ablation-3: no hyperblob_update_loop (means/covs/rot/trans) — hyperblobs stay fixed during tracking.
...
def process_video(video_name, subsampling_percentage=100.0, subsampled_indices=None, run_save_dir=None):
    ...
    try:
        ...
        # Ablation-3: inflate k-means hyperblob covariances (weak cluster-level coupling in the generative init)
        from genjax import ChoiceMapBuilder as C
...
if __name__ == "__main__":
    ...
    _parser = argparse.ArgumentParser(
        description="DAVIS DINO ablation-3: same as ablation-2 but frozen hyperblob Gibbs during tracking"
    )
    davis_run_cli.add_use_sam_args(_parser)
    davis_run_cli.add_save_3wide_video_args(_parser)
    _args = _parser.parse_args()
    davis_run_cli.configure_experiment_module(sys.modules[__name__], _args, "ablation_3")
    ...
    print(f"{'='*80}")
    print(f"DINO ablation-3 (ablation-2 + frozen hyperblob Gibbs during tracking) - Processing {len(VIDEO_NAMES)} videos")
    print(f"  Hyperblobs: {NUM_HYPERBLOBS_ORIGINAL}; Psi_H = 1e6·I; inflated k-means hyperblob covs; frozen hyperblob Gibbs (tracking)")
