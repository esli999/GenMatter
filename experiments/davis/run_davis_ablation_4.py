# GenMatter Tracking with DINO Features — DAVIS ablation-4 (ablation-3 + empirical Ψ_H like ablation-1).
# See run_davis_ablation_3.py for frozen hyperblob Gibbs; run_davis_ablation.py for empirical Ψ_H.
...
EXPERIMENT_SAVE_DIR = str(config.DAVIS_ABLATION_4_OUTPUT_DIR_SAM)

# Model hyperparameters
NUM_BLOBS = 500
# Ablation-4: same as ablation-3 except Ψ_H = median k-means hyperblob cov (ROI), not 1e⁶·I.
...
def blob_tracking_gibbs_dino(key, genmatter_state):
    """Single-frame Gibbs updates (ablation-4: hierarchical blob mean/vel; hyperblob Gibbs frozen during tracking)."""
    ...
    # Ablation-4: no hyperblob_update_loop (means/covs/rot/trans) — hyperblobs stay fixed during tracking.
...
def process_video(video_name, subsampling_percentage=100.0, subsampled_indices=None, run_save_dir=None):
    ...
    try:
        ...
        # Ablation-4: empirical Ψ_H from raw k-means hyperblob covs (ablation-1), before inflating init hyperblob covs.
        empirical_Psi_H = jnp.median(
            kmeans_chm['hyperblobs', 'hyperblob_covs'][roi_hyperblob_indices], axis=0
        )

        # Inflate k-means hyperblob covariances (weak cluster-level coupling in the generative init; same as ablation-2/3)
        from genjax import ChoiceMapBuilder as C
...
if __name__ == "__main__":
    ...
    _parser = argparse.ArgumentParser(
        description="DAVIS DINO ablation-4: ablation-3 + empirical Ψ_H (median k-means hyperblob covs, ablation-1 style)"
    )
    davis_run_cli.add_use_sam_args(_parser)
    davis_run_cli.add_save_3wide_video_args(_parser)
    _args = _parser.parse_args()
    davis_run_cli.configure_experiment_module(sys.modules[__name__], _args, "ablation_4")
    ...
    print(f"{'='*80}")
    print(f"DINO ablation-4 (ablation-3 + empirical Ψ_H) - Processing {len(VIDEO_NAMES)} videos")
    print(f"  Hyperblobs: {NUM_HYPERBLOBS_ORIGINAL}; Psi_H = median k-means hyperblob cov (ROI); inflated k-means hyperblob covs; frozen hyperblob Gibbs (tracking)")
